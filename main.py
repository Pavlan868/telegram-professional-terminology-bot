...
async def task(message: Message):
    ...
    if not q.get("options"):
        # Текстовый вопрос (без вариантов)
        text = (
            f"✍️ **Вопрос {index+1}/{total}**\n\n"
            f"{q['question']}\n\n"
            f"```\n{q.get('code', '')}\n```\n\n" if q.get('code') else ""
            f"_Введите ответ текстовым сообщением_"
        )
        await message.answer(text, parse_mode=None)
        # Не отправляем кнопки!
        return
...
async def glossary_search(message: Message):
    # Игнорируем, если это не поиск (кнопки меню, команды)
    if message.text in [
        "📚 Обучение", "🧠 Задание", "📖 Справочник", "🏆 Достижения",
        "🔄 Сменить язык", "🔁 Повторить обучение", "🧪 Повторить тест", "⚙️ Админка"
    ]:
        return
    ...
    for block in DATA.get("blocks", []):
        if lang and block.get("language") != lang:
            continue  # Фильтр по текущему языку (опционально)
    ...
    if found:
        ...
        for i, t in enumerate(found[:10], 1):  # Показываем первые 10
...
async def handle_text_answer(message: Message):
    # Игнорируем кнопки меню и команды
    if message.text in [
        "📚 Обучение", "🧠 Задание", "📖 Справочник", "🏆 Достижения",
        "🔄 Сменить язык", "🔁 Повторить обучение", "🧪 Повторить тест", "⚙️ Админка"
    ]:
    ...
    if attempt["index"] < attempt["total"]:
        # Вызываем существующую логику отправки следующего вопроса
        # (адаптируй под свою структуру — например, скопируй код из task())
        await task(message)  # Или твоя функция отправки следующего
    else:
...
async def admin_all_stats(callback: CallbackQuery):
    await callback.answer()
    from database import get_all_users_stats  # Импортируй, если не импортировано
    ...
    if stats["inactive"]:
        msg += f"\n⚠️ **Неактивные (>7 дней): {len(stats['inactive'])}**\n"
        for u in stats["inactive"]:
            uid = u.get("user_id")
            name = u.get("username") or u.get("first_name") or f"ID:{uid}"
            last_seen = u.get("last_seen")
            if last_seen:
                date_str = last_seen.strftime("%d.%m") if hasattr(last_seen, "strftime") else str(last_seen)
