...
async def task(message: Message):
    ...
    block = next(
        (b for b in DATA["blocks"] 
         if b["id"] == current_block_id and b["language"] == lang), 
        None
    )
...
async def handle_repeat_block_selection(callback: CallbackQuery):
    ...
    block = next(
        (b for b in DATA["blocks"] 
         if b["id"] == block_id and b["language"] == lang), 
        None
    )
    if not block or not block.get("terms"): 
...
async def admin_add_start(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=l, callback_data=f"admin_add_lang_{l}")] 
            for l in langs
        ]
    )
...
async def admin_add_lang(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=f"#{b['id']} {b['title'][:15]}", callback_data=f"admin_add_block_{b['id']}")] 
            for b in blocks
        ]
    )
...
async def admin_del_start(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=l, callback_data=f"admin_del_lang_{l}")] 
            for l in langs
        ]
    )
...
async def admin_del_lang(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=f"#{b['id']} {b['title'][:15]}", callback_data=f"admin_del_block_{b['id']}")] 
            for b in blocks
        ]
    )
