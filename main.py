# main.py
# Версия 16.0 - FIX: row_width, admin stats import, parse_mode, strings
import asyncio
import json
import logging
import os
...
# ✅ ИСПРАВЛЕНО: Убран row_width (ломает aiogram 3.x)
...
async def learn(message: Message):
    uid = message.from_user.id
    lang = load_user_language(uid)
    if not lang: return await message.answer("Сначала выбери язык!", reply_markup=get_language_keyboard(), parse_mode=None)
    progress = load_progress(uid)
    lang_data = ensure_user_data(progress, lang)
    block = next((b for b in DATA["blocks"] if b["id"] == lang_data.get("current_block", FIRST_BLOCK_ID[lang]) and b["language"] == lang), None)
    if not block or not block.get("terms"): return await message.answer("📭 Нет терминов.", parse_mode=None)
...
async def task(message: Message):
    ...
    progress = load_progress(uid)
    lang_data = ensure_user_data(progress, lang)
    if lang_data.get("current_attempt"): return await message.answer("❗ Тест уже идет.", parse_mode=None)
    ...
    if not block or not block.get("tasks"): return await message.answer("📭 Нет заданий.", parse_mode=None)
...
async def repeat_learn(message: Message):
    ...
    progress = load_progress(uid)
    lang_data = ensure_user_data(progress, lang)
    if lang_data.get("current_attempt"): return await message.answer("❗ Сначала заверши тест.", parse_mode=None)
    ...
    if not blocks_to_show: return await message.answer("📭 Нет тем.", parse_mode=None)
...
async def handle_repeat_block_selection(callback: CallbackQuery):
    ...
    except: return await callback.answer("❌ Ошибка.", parse_mode=None)
    block = next((b for b in DATA["blocks"] if b["id"] == block_id and b["language"] == lang), None)
    if not block or not block.get("terms"): return await callback.message.answer("📭 Нет терминов.", parse_mode=None)
    terms_parts = []
    for t in block["terms"]:
        example_block = "```\n" + t["example"] + "\n```" if t.get("example") else ""
        terms_parts.append(f"**{t['term']}**\n_{t['definition']}_\n{example_block}")
...
async def repeat_test(message: Message):
    ...
    if not all_questions: return await message.answer("📭 Нет вопросов.", parse_mode=None)
...
async def handle_inline_answer(callback: CallbackQuery):
    ...
    if not attempt: return await callback.message.answer(" Нет теста.", parse_mode=None)
    idx = attempt["index"]
...
async def admin_reset(callback: CallbackQuery):
    ...
    uid = callback.from_user.id
    lang = load_user_language(uid)
    if not lang: return await callback.message.answer("❌ Сначала выбери язык.", parse_mode=None)
    progress = load_progress(uid)
...
async def admin_show_stats(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("❌ Введите числовой ID.", parse_mode=None); return
    profile = get_user_profile(target_uid)
    if not profile:
        await message.answer("❌ Пользователь не найден.", parse_mode=None); return
...
async def admin_all_stats(callback: CallbackQuery):
    await callback.answer()
    stats = get_all_users_stats() # <-- ИСПОЛЬЗУЕТСЯ ИМПОРТИРОВАННАЯ ФУНКЦИЯ
...
async def admin_add_options(message: Message, state: FSMContext):
    ...
    if len(options) < 2: return await message.answer("❌ Минимум 2 варианта.", parse_mode=None)
...
async def admin_add_correct(message: Message, state: FSMContext):
    try:
        ...
        if not (0 <= idx < len(data["options"])): return await message.answer(f"❌ Число от 1 до {len(data['options'])}.", parse_mode=None)
    ...
    except: await message.answer("❌ Ошибка ввода.", parse_mode=None)
...
async def handle_unknown(message: Message):
    ...
    if lang:
        await message.answer("❓ Используй кнопки или /start", reply_markup=get_main_keyboard(message.from_user.id), parse_mode=None)
    else:
        await message.answer("👋 Нажми /start", reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="/start")]], resize_keyboard=True), parse_mode=None)
